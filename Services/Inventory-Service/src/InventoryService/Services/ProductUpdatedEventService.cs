...
namespace InventoryService.Services {
    public class ProductUpdatedEventService : IEventService {

        private ISubscriptionClient _subscriptionClient;
        private readonly IOptions<ServiceBus> _serviceBusOptions;
        private readonly IMediator _mediator;
        private readonly ILogger<ProductUpdatedEventService> _logger;
        ...
        public ProductUpdatedEventService (IMediator mediator, IOptions<ServiceBus> serviceBusOptions, ILogger<ProductUpdatedEventService> logger) {
            _mediator = mediator ??
        ...
        public async Task UnSubscribe (CancellationToken token = default (CancellationToken)) {
            ...
            try {
                ...
                if (await client.SubscriptionExistsAsync (_serviceBusOptions.Value.ProductUpdatedTopic, _serviceBusOptions.Value.ProductUpdatedSubscription, token).ConfigureAwait(false)) {
                    await client.DeleteSubscriptionAsync (_serviceBusOptions.Value.ProductUpdatedTopic, _serviceBusOptions.Value.ProductUpdatedSubscription, token).ConfigureAwait(false);
                }

                //Log information
                _logger.LogInformation ($"Stopped Subscription to Topic : {_serviceBusOptions.Value.ProductUpdatedTopic} , Subscription Name : { _serviceBusOptions.Value.ProductUpdatedSubscription}");

            } catch (Exception ex) {
                _logger.LogError ($"Error in UnSubscribing from topic {_serviceBusOptions.Value.ProductUpdatedTopic}, ex - {ex}");
                throw;
            } finally {
                await client.CloseAsync ().ConfigureAwait(false);
            }
